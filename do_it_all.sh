...
for f in ${LIST}; do
  ...
  echo -ne "\033]0;${USER}@${HOSTNAME} [$f]:make -kj${NPROC} defconfig $(date)\007"
  make "-kj${NPROC}" defconfig
  ARCH_PACKAGES=$(sed -ne '/CONFIG_TARGET_ARCH_PACKAGES=/{s/.*=\|\"//g;p}' .config)
  [ "$ARCH_PACKAGES" != "$PREV_ARCH_PACKAGES" ] && {

    echo -ne "\033]0;${USER}@${HOSTNAME} [$f]:make dirclean $(date)\007"
    make dirclean
#    echo -ne "\033]0;${USER}@${HOSTNAME} [$f]:make package/postgresql/host/clean $(date)\007"
#    make package/postgresql/host/clean

    echo -ne "\033]0;${USER}@${HOSTNAME} [$f]:rm bin/\* build_dir/\* staging_dir/t\* staging_dir/packages $(date)\007"
    rm -rf bin/* build_dir/t* staging_dir/t* staging_dir/packages
    PREV_ARCH_PACKAGES=""
  }
  echo -ne "\033]0;${USER}@${HOSTNAME} [$f]:make -kj${NPROC} $(date)\007"
  make "-kj${NPROC}" || {
